...
# What is the highest number of attendees? `max_3attendees`
max_3attendees <- max(num_attendees, na.rm = TRUE)
...
count_in_location <- function(location) {
  occured_protests <- sum(str_detect(locations, location))
  paste("There were", occured_protests, "protests in", location, ".")
}
...
most_recent <- as.Date(max(dates))

# What is the earliest date in the dataset? `earliest`
earliest <- as.Date(min(dates))
...
in_2020 <- dates[c(dates >= "2020-01-01", dates < "2020-12-31")]

# Create a vector of the dates that are in 2019. `in_2019`

in_2019 <- dates[c(dates >= "2019-01-01", dates < "2020-12-31")]
...
count_on_date <- function (date) {
  protest_on_date <- sum(str_detect(dates, date))
  return(paste("There were", protest_on_date, "protests on", date, "."))
}
...
high_level_purpse <- trimws(gsub("\\(.*", "", purpose))
...
loc_date_article <- protests[, c('Date','Total.Articles')]
View(loc_date_article)

search_location_date <- function(date) {
  articles <- sum(str_detect(loc_date_article, date))
  return (paste("There are", articles, "about the protests in", location, "on", date))
}
search_location_date("Seattle, WA", "2020-01-01")
